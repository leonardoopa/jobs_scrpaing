...
def health_check():
    ...
    return {
        "status": "online",
        "database": "PostgreSQL",
        "message": "API is running at 100% capacity!",
    }
...
def get_jobs(
    ...
    location: str = Query(
        None, description="Filter by city or location (e.g., Recife)"
    ),
    platform: str = Query(
        None, description="Filter by source platform (e.g., InfoJobs)"
    ),
    ...
    if role:
        query = query.filter(
            or_(Vaga.titulo.ilike(f"%{role}%"), Vaga.cargo_buscado.ilike(f"%{role}%"))
        )
    ...
    return {"total_found": len(found_jobs), "jobs": found_jobs}
